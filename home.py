...
pages = []
for i in range(4):
    pages.append(tk.Frame(root, bg="black"))
...
def basicPageSetup(pageNo):
    ...
    back_button = tk.Button(pages[pageNo], image=back_img, bg="black", bd=0, highlightthickness=0,
                            activebackground="black", command=goBack)
    back_button.image = back_img
    back_button.place(x=10, y=10)

    heading = tk.Label(pages[pageNo], fg="white", bg="black", font="Arial 20 bold", pady=10)
    heading.pack()

    content = tk.Frame(pages[pageNo], bg="black", pady=20)
    content.pack(expand="true", fill="both")

    left_frame = tk.Frame(content, bg="black")
    left_frame.grid(row=0, column=0, sticky="nsew")

    right_frame = tk.LabelFrame(content, text="Detected Criminals", bg="black", font="Arial 20 bold", bd=4,
...
# Function to handle canvas scroll event
...
def getPage1():
    ...
    basicPageSetup(1)
    heading.configure(text="Register Criminal",font="Arial 30 bold",fg="#9C27B0" )
    right_frame.configure(text="Enter Criminal Details",fg="#9C27B0")

    btn_grid = tk.Frame(left_frame, bg="black")
    btn_grid.pack()

    tk.Button(btn_grid, text="Select Images", command=lambda: selectMultiImage(opt_menu, menu_var), font="Arial 15 bold", bg="#9C27B0",
    ...
    scrollbar = tk.Scrollbar(right_frame, command=canvas.yview, width=20, troughcolor="black", bd=0,
    ...
    tk.Label(scroll_frame, text="* Required Fields", bg="black", fg="white", font="Arial 13 bold").pack()
    # Adding Input Fields
    ...
    for i, field in enumerate(input_fields):
        ...
        label = tk.Text(row, width=20, height=1, bg="black", fg="#ffffff", font="Arial 13", highlightthickness=0, bd=0)
        label.insert("insert", field)
        label.pack(side="left")

        if (required[i] == 1):
            label.tag_configure("star", foreground="white", font="Arial 13 bold")
        ...
        if (i != ip_len - 1):
            ...
            opt_menu.configure(font="Arial 13", bg="#9C27B0", fg="white", bd=0, highlightthickness=0,
    ...
    tk.Button(scroll_frame, text="Register Criminal", command=lambda: register(entries, required, menu_var), font="Arial 15 bold",
              bg="#9C27B0", fg="white", pady=10, padx=30, bd=0, highlightthickness=0, activebackground="#673AB7",
...
def showCriminalProfile(name):
    ...
    tk.Label(top, text="Criminal Profile", fg="white", bg="black", font="Arial 20 bold", pady=10).pack()

    content = tk.Frame(top, bg="black", pady=20)
    content.pack(expand="true", fill="both")
    ...
    if id is not None:
        ...
        img = Image.fromarray(img)
        img = ImageTk.PhotoImage(img)
        img_label = tk.Label(content, image=img, bg="black")
        img_label.image = img
        img_label.grid(row=0, column=0)

        info_frame = tk.Frame(content, bg="black")
        info_frame.grid(row=0, column=1, sticky='w')

        for i, item in enumerate(crim_data.items()):
            tk.Label(info_frame, text=item[0], pady=15, fg="white", font="Arial 15 bold", bg="black").grid(row=i, column=0,
                                                                                                           sticky='w')
            tk.Label(info_frame, text=":", fg="white", padx=50, font="Arial 15 bold", bg="black").grid(row=i, column=1)
            val = "---" if (item[1] == "") else item[1]
            tk.Label(info_frame, text=val.capitalize(), fg="white", font="Arial 15", bg="black").grid(row=i, column=2,
                                                                                                       sticky='w')
    else:
...
def getPage2():
    ...
    tk.Button(btn_grid, text="Select Image", command=selectImage, font="Arial 15 bold", padx=20, bg="#9C27B0",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#673AB7",
              activeforeground="white").grid(row=0, column=0, padx=25, pady=25)
    tk.Button(btn_grid, text="Recognize", command=startRecognition, font="Arial 15 bold", padx=20, bg="#9C27B0",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#673AB7",
...
def getPage3():
    ...
    basicPageSetup(3)
    heading.configure(text="Video Surveillance", font="Arial 35 bold",fg="#9C27B0")
    right_frame.configure(text="Detected Criminals", fg="#9C27B0")
...
######################################## Home Page ####################################
tk.Label(pages[0], text="CRIMINAL  DETECTION  SYSTEM", fg="#9C27B0", bg="black",
      font="Impact 35 bold", pady=30).pack()

logo = tk.PhotoImage(file = "logo.png")
tk.Label(pages[0], image=logo, bg="black").pack()

btn_frame = tk.Frame(pages[0], bg="black", pady=30)
btn_frame.pack()

tk.Button(btn_frame, text="Register Criminal", command=getPage1)
tk.Button(btn_frame, text="Scan Criminal", command=getPage2)
tk.Button(btn_frame, text="CCTV Surveillance", command=getPage3)

tk.Label(pages[0], text="-G YESHWANTH GOUD \n HU21CSCI0200001 \n BSC.CSCS", fg="#9C27B0", bg="black",
      font="Arial 20 bold", pady=30).pack()

for btn in btn_frame.winfo_children():
    btn.configure(font="Arial 20", width=17, bg="#9C27B0", fg="white",
        pady=15, bd=0, highlightthickness=0, activebackground="#673AB7", activeforeground="white")
    btn.pack(pady=30)
